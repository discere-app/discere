...
class SpeciesRepository {
  ...
  static const String _selectClause =
      ...
      (SELECT GROUP_CONCAT(cn.name, ';') FROM (SELECT cn.name FROM common_names cn WHERE cn.entity_id = $speciesAlias.$columnSpeciesId AND cn.language = 'de' ORDER BY (cn.country IS NULL) DESC, cn.is_preferred DESC, cn.rank ASC LIMIT 20) sub) AS ${speciesAlias}_$columnSpeciesCommonNameDe,
      (SELECT GROUP_CONCAT(cn.name, ';') FROM (SELECT cn.name FROM common_names cn WHERE cn.entity_id = $speciesAlias.$columnSpeciesId AND cn.language = 'en' ORDER BY (cn.country IS NULL) DESC, cn.is_preferred DESC, cn.rank ASC LIMIT 20) sub) AS ${speciesAlias}_$columnSpeciesCommonNameEn,
      (SELECT GROUP_CONCAT(cn.name, ';') FROM (SELECT cn.name FROM common_names cn WHERE cn.entity_id = $speciesAlias.$columnSpeciesId AND cn.language = 'fr' ORDER BY (cn.country IS NULL) DESC, cn.is_preferred DESC, cn.rank ASC LIMIT 20) sub) AS ${speciesAlias}_$columnSpeciesCommonNameFr,
      (SELECT GROUP_CONCAT(cn.name, ';') FROM (SELECT cn.name FROM common_names cn WHERE cn.entity_id = $speciesAlias.$columnSpeciesId AND cn.language = 'es' ORDER BY (cn.country IS NULL) DESC, cn.is_preferred DESC, cn.rank ASC LIMIT 20) sub) AS ${speciesAlias}_$columnSpeciesCommonNameEs,
