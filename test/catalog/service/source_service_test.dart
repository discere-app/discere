...
void main() {
  ...
  group('SourceService.getAllSources', () {
    test('appends the synthetic Wikipedia source after the curated ones', () async {
      final curated = [
        const Source(
          id: 'fishbase',
          name: 'FishBase',
  ...
  group('SourceService.getDistinctLicenses', () {
    test('adds the Wikipedia CC BY-SA license if not already present', () async {
      when(sourceRepository.distinctLicenses()).thenAnswer(
        (_) async => [(key: 'CC BY-NC', licenseUrl: null)],
      );

      final licenses = await service.getDistinctLicenses();

      expect(licenses.map((l) => l.key), containsAll(['CC BY-NC', 'CC BY-SA 4.0']));
    });

    test('does not duplicate the Wikipedia license if already present', () async {
      when(sourceRepository.distinctLicenses()).thenAnswer(
        (_) async => [
          (
            key: 'CC BY-SA 4.0',
